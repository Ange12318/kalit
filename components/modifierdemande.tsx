  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const d = demande;
        setReference(d.REF_DEMANDE || '');
        setAutorisation(d.AUT_DEMANDE || '');
        setDossier(d.NUM_DOSSIER || '');
        setNature(d.NATURE_DEMANDE || '');
        setExportateurId(d.ID_EXPORTATEUR || '');
        setProduitId(d.ID_PRODUIT || '');
        setVille(d.VILLE_EXPORTATEUR || 'ABIDJAN');
        setCampagne(d.CAMP_DEMANDE || '');
        setDateAutorisation(d.DATEEMI_DEMANDE ? d.DATEEMI_DEMANDE.slice(0, 10) : today);
  ...
  const enregistrerModifications = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Erreur serveur: ' + (error.error || 'Erreur inconnue'));
      }
    } catch (e) {
      alert('Erreur réseau');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-[#0d2d53] text-white p-4 shadow-lg">
        ...
        <p className="text-sm opacity-90">Modifier la demande d'analyse #{demandeId}</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 space-y-6">
          ...
          <div className="mt-8">
                    ...
                    <tr key={i} className="border-b">
                      ...
                      <td className="px-4 py-2 text-center">
                        <div className="flex gap-2 justify-center">
                          {i > 0 && (
                            <button onClick={() => dupliquerLotPrecedent(i)} className="text-blue-600 hover:text-blue-800 text-sm" title="Dupliquer le lot précédent">
                              Dupliquer
                            </button>
                          )}
                          {lots.length > 1 && (
                            <button onClick={() => supprimerLigne(i)} className="text-red-600 hover:text-red-800 text-sm" title="Supprimer cette ligne">
                              Supprimer
                            </button>
                          )}
                        </div>
            ...
            <div className="mt-6 flex justify-end gap-4">
              ...
              <button onClick={enregistrerModifications} disabled={saving} className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-10 rounded flex items-center gap-2">
          ...
          <div className="flex justify-between mt-10">
            ...
            <button onClick={onNavigateBack} className="bg-gray-400 hover:bg-gray-500 text-white font-bold py-3 px-8 rounded">Annuler</button>
          </div>
        </div>
      </div>
    </div>
