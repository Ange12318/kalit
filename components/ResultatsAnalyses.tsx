...
const ResultatsAnalyses: React.FC<ResultatsAnalysesProps> = ({ onNavigateBack }) => {
  ...
  const [statutValidation, setStatutValidation] = useState('all'); // Nouveau filtre pour le statut
  ...
  const getRowColorClass = (analyse: AnalyseCacao) => {
    if (analyse.VALIDER_ANALYSE_KKO === 1) {
      ...
      if (analyse.CONFORME === 1) {
        return 'bg-green-50 hover:bg-green-100'; // Vert clair pour validé + conforme
      } else {
        return 'bg-orange-50 hover:bg-orange-100'; // Orange clair pour validé + non conforme
      }
    } else {
      return 'bg-red-50 hover:bg-red-100'; // Rouge clair pour non validé
    }
  };
  ...
  const chargerAnalyses = async (force = false) => {
    ...
    try {
      ...
      console.log('Données reçues:', data.length, 'analyses'); // Debug
      ...
      if (filteredData.length === 0) {
        setSuccess('Aucune analyse trouvée pour les critères sélectionnés');
      } else {
        setSuccess(`${filteredData.length} analyse(s) trouvée(s)`);
      }
  ...
  const toggleSelectAll = () => {
    if (selectedAnalyses.length === filteredAnalyses.length) {
      ...
      setSelectedAnalyses(filteredAnalyses.map(a => a.ID_ANALYSE_KKO));
    }
  };
  
  const toggleSelectAnalysis = (id: number) => {
  ...
  const validerAnalyses = async (valider: boolean) => {
    ...
    try {
      console.log('Validation des analyses:', selectedAnalyses, 'valider:', valider); // Debug
      ...
      console.log('Réponse validation:', response.status); // Debug
      ...
      console.log('Résultat validation:', result); // Debug
      ...
      console.error('Erreur validation:', err); // Debug
  ...
  const ouvrirEdition = (analyse: AnalyseCacao) => {
    setEditingAnalysis(analyse);
  ...
  const calculerTotalDefectueuses = (analyse: AnalyseCacao) => {
    return (
  ...
  const getBadgeStatut = (analyse: AnalyseCacao) => {
    if (analyse.VALIDER_ANALYSE_KKO === 1) {
      if (analyse.CONFORME === 1) {
        return (
      ...
      <div className="bg-white p-6 rounded-b-xl shadow-lg border space-y-6">
        ...
        <div className="mb-6 pb-6 border-b">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            ...
            <div className="space-y-1">
              <label className="font-medium text-gray-600">Date Analyse (Début)</label>
            ...
            <div className="space-y-1">
              <label className="font-medium text-gray-600">Date Analyse (Fin)</label>
              <div className="relative">
                <input 
                  id="dateFinInput"
                  type="date" 
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <div className="mt-3 pt-3 border-t border-blue-200">
              <div className="flex justify-between items-center">
                <div>
                  <span className="font-semibold text-gray-700">Nombre Total de lots:</span>
                  <p className="font-bold text-lg text-blue-800">{filteredAnalyses.length}</p>
                </div>
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-100 uppercase text-xs text-gray-600">
                <tr>
                  ...
                  <th className="p-3 font-semibold tracking-wider text-center">Date analyse</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td colSpan={23} className="text-center py-10">
                      <div className="flex justify-center">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>
                      </div>
                      <p className="text-gray-500 mt-2">Chargement des analyses...</p>
                    </td>
                  </tr>
                ) : filteredAnalyses.length === 0 ? (
                  <tr>
                    <td colSpan={23} className="text-center py-20">
                  ...
                  filteredAnalyses.map((analyse) => (
                    <tr 
                      key={analyse.ID_ANALYSE_KKO} 
                      className={`transition-colors duration-150 ${getRowColorClass(analyse)}`}
                    >
                      <td className="p-3 text-center">
                        <input 
                          type="checkbox" 
                          checked={selectedAnalyses.includes(analyse.ID_ANALYSE_KKO)}
                          onChange={() => toggleSelectAnalysis(analyse.ID_ANALYSE_KKO)}
                          className="form-checkbox h-4 w-4"
                          disabled={analyse.VALIDER_ANALYSE_KKO === 1} // Désactiver si déjà validé
                        />
                      </td>
                      <td className="p-3 font-mono font-medium">{analyse.CODE_SECRET_CODIFICATION}</td>
                      <td className="p-3">{analyse.NOM_ANALYSEUR || 'N/A'}</td>
                      ...
                      <td className="p-3 text-center">
                        <div className="flex justify-center space-x-2">
                          <button 
                            ...
                            title="Modifier"
                            disabled={analyse.VALIDER_ANALYSE_KKO === 1} // Désactiver si validé
                          >
                          ...
                          <button 
                            onClick={() => {
                              if (analyse.VALIDER_ANALYSE_KKO === 1) {
                                setError('Cette analyse est déjà validée');
                                return;
                              }
                              setSelectedAnalyses([analyse.ID_ANALYSE_KKO]);
                              setValidationType(analyse.CONFORME ? 'reject' : 'validate');
                              setShowValidationDialog(true);
                            }}
                            className={`p-1 rounded hover:bg-gray-100 ${
                              analyse.VALIDER_ANALYSE_KKO === 1 ? 'text-gray-400 cursor-not-allowed' :
                              analyse.CONFORME ? 'text-green-600 hover:text-green-800' : 'text-red-600 hover:text-red-800'
                            }`}
                            title={analyse.VALIDER_ANALYSE_KKO === 1 ? 'Déjà validée' : (analyse.CONFORME ? 'Valider' : 'Rejeter')}
                            disabled={analyse.VALIDER_ANALYSE_KKO === 1}
                          >
          ...
          <div className="text-sm text-gray-600">
            {selectedAnalyses.length > 0 && (
              <span className="font-medium">
                {selectedAnalyses.length} analyse(s) sélectionnée(s)
              </span>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full">
            ...
            <p className="text-gray-700 mb-6">
              ...
              {validationType === 'validate' ? ' Elles seront marquées comme validées.' : ' Elles seront marquées comme non conformes.'}
            </p>
            <div className="flex justify-end space-x-4">
              ...
              <button 
                onClick={() => validerAnalyses(validationType === 'validate')}
                disabled={loading}
